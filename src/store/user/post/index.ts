...
type PostStore = {
    ...
    userHasMore: boolean
    error: string | null
    reportError: string | null // Add new error state for reporting
    fetchPosts: (initial?: boolean) => Promise<void>
    createPost: (content: string, image: File | null) => Promise<void>
    deletePost: (postId: string) => Promise<void>
    toggleLike: (postId: string) => Promise<void>
...
export const usePostStore = create<PostStore>((set, get) => ({
    ...
    error: null,
    selectedPostId: null,
    reportError: null, // Initialize report error state
    ...
    reportPost: async (postId, reason) => {
        try {
            await axiosPrivate.post(`/posts/${postId}/report`, {
                reason,
            })
            ...
            if (axiosError.response?.status === 409) {
                set({ reportError: "You have already reported this post" })
            } else {
                set({ reportError: "Failed to report post" })
            }
